...
export default function Index({ products, filters = {} }) {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (!workbook.SheetNames.includes("Generators")) {
                        ...
                        reject(
                            new Error(
                                'The Excel file must contain a "Generators" sheet.'
                            )
                        );
                        return;
                    }
                    ...
                    if (jsonData.length < 2) {
                        ...
                        console.error(
                            `Expected: At least 2 rows (headers at row 1, data at row 2)`
                        );
                        ...
                        reject(
                            new Error(
                                "The Excel file must have at least 2 rows (headers at row 1, data at row 2)."
                            )
                        );
                        return;
                    }
                    ...
                    if (stockIdColumnIndex === -1) {
                        console.warn(
                            "⚠️  Stock ID column not found in headers!"
                        );
                        console.warn(
                            'Searched for: "Stock ID", "StockID", "Unit ID", "UnitID"'
                        );
                        ...
                        console.log(
                            `   Column: ${String.fromCharCode(
                                65 + stockIdColumnIndex
                            )}`
                        );
                        console.log(`   Index: ${stockIdColumnIndex}`);
                        console.log(
                            `   Header: "${headers[stockIdColumnIndex]}"`
                        );
                    }
                    console.groupEnd();
                    ...
                    console.log(
                        `Total data rows (starting from row 2): ${rows.length}`
                    );

                    const stockIdIndex =
                        stockIdColumnIndex !== -1 ? stockIdColumnIndex : 26;
                    const stockIdAnalysis = rows
                        .slice(0, 10)
                        .map((row, idx) => {
                    ...
                    if (emptyRows.length > 0) {
                        console.warn(
                            `Empty rows skipped: ${
                                emptyRows.length
                            } (rows: ${emptyRows.slice(0, 10).join(", ")}${
                                emptyRows.length > 10 ? "..." : ""
                            })`
                        );
                    }
                    ...
                    const products = validRows
                        .map((row, originalIndex) => {
                            ...
                            const product = {
                                ...
                                power: row[27]
                                    ? parseInt(row[27]) || null
                                    : null,
                                engine_speed: row[28]
                                    ? parseInt(row[28]) || null
                                    : null,
                                radiator_design_temp: row[29]
                                    ? parseInt(row[29]) || null
                                    : null,
                                frequency: row[30]
                                    ? parseInt(row[30]) || null
                                    : null,
                                full_load_amps: row[31]
                                    ? parseInt(row[31]) || null
                                    : null,
                                ...
                                total_cost: row[37]
                                    ? parseFloat(row[37]) || null
                                    : null,
                                retail_cost: row[38]
                                    ? parseFloat(row[38]) || null
                                    : null,
                                tariff_cost: row[39]
                                    ? parseFloat(row[39]) || null
                                    : null,
                    ...
                    console.log(
                        `Valid products (with Stock ID): ${validProducts.length}`
                    );
                    if (skippedProducts.length > 0) {
                        console.error(
                            `❌ Products without Stock ID: ${skippedProducts.length}`
                        );
                        console.table(
                    ...
                    if (skippedCount > 0) {
                        console.warn(
                            `⚠️  WARNING: ${skippedCount} row(s) were skipped:`
                        );
                        if (emptyRowCount > 0) {
                        ...
                        if (missingStockIdCount > 0) {
                            console.error(
                                `   - ${missingStockIdCount} row(s) missing Stock ID`
                            );
                        }
                    }
                    ...
                    reject(
                        new Error("Error parsing Excel file: " + error.message)
                    );
                }
            };
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            console.log(
                `✅ Parsing complete. Products ready to import: ${productsData.length}`
            );
            ...
            if (productsData.length === 0) {
                ...
                console.groupEnd();
                alert(
                    "No valid products found in the Excel file. All products must have a Stock ID value."
                );
            ...
            console.log("📋 Products by type:", {
                Generators: productsData.filter(
                    (p) => p.product_type === "Generators"
                ).length,
                Switch: productsData.filter((p) => p.product_type === "Switch")
                    .length,
                "Docking Stations": productsData.filter(
                    (p) => p.product_type === "Docking Stations"
                ).length,
                Other: productsData.filter((p) => p.product_type === "Other")
                    .length,
            });
            console.groupEnd();
    ...
    return (
        <AdminLayout>
            ...
            {flash?.import_errors &&
                Array.isArray(flash.import_errors) &&
            ...
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
                <strong>Excel Format:</strong> The file must contain a
                "Generators" sheet with headers in row 1 (A1 onwards) and data
                starting from row 2.
            </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    >
                        ...
                        <button
                            type="submit"
                            disabled={
                                processing || processingExcel || !data.file
                            }
            ...
            <div className="mb-4 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                ...
                {products && (
                    <div className="text-sm text-slate-600">
                        <strong>Total Records:</strong> {products.total || 0}{" "}
                        product(s)
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm overflow-x-auto">
                <table className="w-full text-left min-w-full">
                    <thead className="bg-slate-50 text-slate-500 text-xs uppercase font-semibold">
                        <tr>
                            <th className="px-4 py-3 w-16 sticky left-0 bg-slate-50 z-10">
                                #
                            </th>
                            <th
                        ...
                        {products.data.length > 0 ? (
                            products.data.map((product, index) => {
                                ...
                                return (
                                    <tr
                                        key={product.id}
                                        className="hover:bg-slate-50 transition border-b border-slate-100 last:border-0 text-sm text-slate-700"
                                    >
                                        <td className="px-4 py-3 text-slate-500 font-medium sticky left-0 bg-white z-10">
                                            {rowNumber}
                                        </td>
                                        ...
                                        <td className="px-4 py-3">
                                            {product.opportunity_name || "-"}
                                        </td>
                                        <td className="px-4 py-3">
                                            {product.hold_expiration_date ||
                                                "-"}
                                        </td>
                                        <td className="px-4 py-3">
                                            {product.brand || "-"}
                                        ...
                                        <td className="px-4 py-3">
                                            {product.ipas_cpq_number || "-"}
                                        </td>
                                        <td className="px-4 py-3">
                                            {product.cps_po_number || "-"}
                                        </td>
                                        <td className="px-4 py-3">
                                            {product.ship_date || "-"}
                                        </td>
